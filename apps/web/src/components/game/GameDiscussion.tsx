...
export function GameDiscussion() {
  const { players, settings, setGameState } = useGameStore()
  ...
  }, [settings.timerEnabled, isPaused, timeRemaining])

  const formatTime = (s: number) =>
    `${Math.floor(s / 60)}:${String(s % 60).padStart(2, '0')}`

  const timerColor =
    timeRemaining > 30 ? 'text-white' : timeRemaining > 10 ? 'text-yellow-400' : 'text-red-400'
  const timerBorder =
    timeRemaining > 30 ? 'border-slate-600' : timeRemaining > 10 ? 'border-yellow-500' : 'border-red-500'
  ...
  return (
    <div className="flex flex-col flex-1 p-4">
        ...
        <div className="flex flex-col items-center mb-8">
          <div
            className={`
              ...
              ${timerBorder} ${timeRemaining <= 10 ? 'animate-pulse' : ''}
            `}
          >
      ...
      {/* Reveal button */}
      <Button variant="primary" size="lg" fullWidth onClick={() => setGameState('reveal')}>
        🎭 Onthul Imposter
      </Button>
    </div>
  )
