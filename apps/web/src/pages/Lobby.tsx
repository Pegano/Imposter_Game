...
export function Lobby() {
  ...
  const { session, players, addPlayer, removePlayer, settings, updateSettings, assignRoles } =
    useGameStore()
  ...
  return (
    <div className="flex flex-col flex-1 p-4 overflow-hidden">
      ...
      {/* ── Game Code (multi-device) ── */}
      {session?.code && (
        <div className="bg-slate-800 rounded-xl p-3 mb-3 text-center shrink-0">
          <p className="text-slate-400 text-xs mb-1">Game code</p>
          <p className="text-xl font-mono font-bold text-primary-400">{session.code}</p>
        </div>
      )}

      {/* ── Settings: 3 knoppen op 1 rij ── */}
      ...
      {/* ── Uitklap: Categorieën (1 kolom) ── */}
      {categoryOpen && (
        <div className="bg-slate-800 rounded-xl mb-2 overflow-hidden shrink-0">
          {/* Alle */}
          <button
            onClick={() => { updateSettings({ categoryId: undefined }); setCategoryOpen(false) }}
          ...
          {categories.map((cat) => (
            <button
              key={cat.id}
              onClick={() => { updateSettings({ categoryId: cat.id }); setCategoryOpen(false) }}
